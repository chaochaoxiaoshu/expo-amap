  ...
  func mapView(_ mapView: MAMapView!, viewFor annotation: MAAnnotation!) -> MAAnnotationView! {
      ...
      let marker = markerManager.getMarker(id: annotation.id)
    {
      if let image = marker.image {
        ...
        var view =
          mapView.dequeueReusableAnnotationView(withIdentifier: reuseId) as? TextAnnotationView
        if view == nil {
          view = TextAnnotationView(
            annotation: annotation, reuseIdentifier: reuseId, textStyle: marker.textStyle,
            textOffset: CGPoint(x: marker.textOffset?.x ?? 0, y: marker.textOffset?.y ?? 0))
        } else {
        ...
        var view =
          mapView.dequeueReusableAnnotationView(withIdentifier: reuseId) as? MAPinAnnotationView
        if view == nil {
